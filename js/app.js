(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".package__slider")) new core(".package__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3.4,
            ...
            speed: 800,
            navigation: {
                prevEl: ".page__package .swiper-button-prev",
                nextEl: ".page__package .swiper-button-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.05,
        ...
        if (document.querySelector(".flights__slider")) new core(".flights__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            loop: true,
            navigation: {
                prevEl: ".page__flights .swiper-button-prev",
                nextEl: ".page__flights .swiper-button-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
    var MotionPathPlugin_gsap, MotionPathPlugin_PropTween, _getUnit, _toArray, MotionPathPlugin_getStyleSaver, MotionPathPlugin_reverting, _xProps = "x,translateX,left,marginLeft,xPercent".split(","), _yProps = "y,translateY,top,marginTop,yPercent".split(","), MotionPathPlugin_DEG2RAD = Math.PI / 180, _getGSAP = function _getGSAP() {
    ...
    const dataLists = document.querySelectorAll(".datalist-search");
    if (dataLists) dataLists.forEach((dataListEl => {
        class DataList {
            constructor(container, input, list, options) {
            ...
            create(filter = "") {
                const list = dataListEl.querySelector(this.list);
                const filterOptions = this.options.filter((d => "" === filter || d.text.includes(filter)));
                if (0 === filterOptions.length) list.classList.remove("active"); else list.classList.add("active");
                list.innerHTML = filterOptions.map((o => `<li id=${o.value}>${o.text}</li>`)).join("");
            }
            addListeners(datalist) {
                const container = dataListEl.querySelector(this.container);
                const input = dataListEl.querySelector(this.input);
                const list = dataListEl.querySelector(this.list);
                container.addEventListener("click", (e => {
                    if (e.target.classList.contains("datalist__input")) container.classList.toggle("active");
                }));
                ...
                list.addEventListener("click", (function(e) {
                    if ("li" === e.target.nodeName.toLocaleLowerCase()) {
        ...
        const datalist = new DataList(".datalist", ".datalist__input", ".datalist__ul", data);
        datalist.create();
        datalist.addListeners(datalist);
    }));
    const airports = document.querySelector(".search__airports");
    const changeDirections = airports.querySelector(".search__svg-directions");
    changeDirections.addEventListener("click", (function(e) {
        const inputFrom = airports.querySelector('input[name="form[from]"]');
        const inputTo = airports.querySelector('input[name="form[To]"]');
        let inputFromValue = inputFrom.value;
        let inputToValue = inputTo.value;
        inputFrom.value = inputToValue;
        inputTo.value = inputFromValue;
        if ("" == inputFrom.value && inputFrom.closest(".datalist").classList.contains("checked")) inputFrom.closest(".datalist").classList.remove("checked");
        if ("" == inputTo.value && inputTo.closest(".datalist").classList.contains("checked")) inputTo.closest(".datalist").classList.remove("checked");
        if ("" != inputFrom.value && !inputFrom.closest(".datalist").classList.contains("checked")) inputFrom.closest(".datalist").classList.add("checked");
        if ("" != inputTo.value && !inputTo.closest(".datalist").classList.contains("checked")) inputTo.closest(".datalist").classList.add("checked");
    }));
    const guestLists = document.querySelector(".guests");
    if (guestLists) {
        class DataList {
            constructor(container, input, list, options) {
                this.container = container;
            ...
            create(filter = "") {
                const list = guestLists.querySelector(this.list);
                const filterOptions = this.options.filter((d => "" === filter || d.text.includes(filter)));
                if (0 === filterOptions.length) list.classList.remove("active"); else list.classList.add("active");
                list.innerHTML = filterOptions.map((o => `<li id=${o.value}>${o.text}</li>`)).join("");
            }
            addListeners(datalist) {
                const container = guestLists.querySelector(this.container);
                const input = guestLists.querySelector(this.input);
                const list = guestLists.querySelector(this.list);
                container.addEventListener("click", (e => {
                    if (e.target.classList.contains("datalist__input")) container.classList.toggle("active");
                }));
        ...
        const data = [ {
            ...
            value: "2",
            text: "2 Guests, Econom"
        }, {
            value: "3",
            text: "1 Guest, Bussiness"
        }, {
            value: "4",
            text: "2 Guests, Bussiness"
        }, {
            value: "5",
            text: "1 Guest, Premium"
        }, {
            value: "6",
            text: "2 Guests, Premium"
        } ];
        const datalist = new DataList(".datalist", ".datalist__input", ".datalist__ul", data);
        datalist.create();
        datalist.addListeners(datalist);
    ...
    document.addEventListener("click", (function(e) {
        if (e.target.classList.contains("datalist__svg")) {
            e.target.closest(".datalist").classList.remove("checked");
            const input = e.target.closest(".datalist").querySelector(".datalist__input");
            input.value = "";
        }
    }));
    ...
    document.addEventListener("selectCallback", (function(e) {
        ...
        const checkedValue = currentSelect.value;
        const filterSlides = document.querySelectorAll("[data-filter]");
        filterSlides.forEach((filterSlide => {
            if (filterSlide.dataset.filter != checkedValue) filterSlide.classList.add("hide");
            if (filterSlide.dataset.filter == checkedValue) filterSlide.classList.remove("hide");
            if ("All" == checkedValue) filterSlide.classList.remove("hide");
            console.log(filterSlide.dataset.filter);
        }));
    }));
    const plain = document.querySelector(".price__plain");
    const destinationPlain = document.querySelector(".destination__plane");
    document.addEventListener("watcherCallback", (function(e) {
        const entry = e.detail.entry;
        const targetElement = entry.target;
        if (targetElement === plain) gsapWithCSS.fromTo(".price__plain", {
            scale: 0
        }, {
            duration: 4,
            transformOrigin: "50% 50%",
            motionPath: {
                path: ".path-line-price",
                align: ".path-line-price",
                alignOrigin: [ .2, .9 ],
                autoRotate: true
            },
            scale: .9,
        ...
        if (targetElement === destinationPlain) gsapWithCSS.fromTo(".destination__plane", {
            scale: 0
        }, {
            duration: 3.6,
            motionPath: {
                path: ".path-line-destination",
                align: ".path-line-destination",
                alignOrigin: [ .2, .9 ],
                autoRotate: 60
            },
            scale: 1,
